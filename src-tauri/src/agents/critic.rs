...
impl CriticAgent {
    ...
    async fn analyze_single_delta(
        ...
        let prompt = format!(
            r#"你是一个专业的批判分析专家。请对以下创新提案进行全面的批判分析：

{}

请从以下维度进行深度批判分析，并以JSON格式返回：
        ...
        if let Some(criticism_array) = json["criticisms"].as_array() {
            for criticism_obj in criticism_array {
                ...
                let criticism = Criticism {
                    delta_index: 0, // 会在调用方设置正确的索引
                    category,
                    message: format!("{}: {}", 
                        criticism_obj["title"].as_str().unwrap_or("批评"),
